...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'add_task' || $action === 'edit_task') {
        ...
        if ($title === '') {
            ...
            mysqli_stmt_bind_param($stmt, "iisssss", $userId, $sub2, $title, $desc, $due2, $time2, $priority, $status);
            mysqli_stmt_execute($stmt);
            $message = "Task added."; $msgType = "ok";
        } else {
            $id = intval($_POST['task_id']);
            $completedAt = $status === 'completed' ? ', completed_at = NOW()' : ', completed_at = NULL';
            $stmt = mysqli_prepare($conn, "UPDATE tasks SET subject_id=?, title=?, description=?, due_date=?, due_time=?, priority=?, status=? $completedAt WHERE id=? AND user_id=?");
            mysqli_stmt_bind_param($stmt, "issssssii", $sub2, $title, $desc, $due2, $time2, $priority, $status, $id, $userId);
            mysqli_stmt_execute($stmt);
    ...
    if ($action === 'delete_task') {
        $id = intval($_POST['task_id']);
        mysqli_query($conn, "DELETE FROM tasks WHERE id=$id AND user_id=$userId");
...
$subjectsForFilter = mysqli_query($conn, "SELECT * FROM subjects WHERE user_id=$userId ORDER BY subject_name");

$pageTitle = "Tasks / To-Do";
$pageSub   = "Manage your assignments, homework and personal tasks.";
$activeNav = "tasks";
...
<div class="card">
  ...
  <table>
    <thead><tr><th></th><th>Task</th><th>Subject</th><th>Due</th><th>Priority</th><th>Status</th><th></th></tr></thead>
    <tbody>
      ...
      <tr>
        <td>
          <form method="POST">
            ...
            <button type="submit" class="btn-icon btn-outline" style="width:26px;height:26px;border-radius:50%;<?php echo $t['status']==='completed'?'background:rgba(46,207,122,.2);border-color:var(--green);color:var(--green)':'border-color:rgba(94,234,212,.45)'; ?>" title="Toggle complete">
