...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'add_subject' || $action === 'edit_subject') {
        ...
        if ($name === '') {
            ...
            $stmt = mysqli_prepare($conn, "UPDATE subjects SET subject_name=?, subject_code=?, instructor=?, units=?, color=? WHERE id=? AND user_id=?");
            mysqli_stmt_bind_param($stmt, "sssdsii", $name, $code, $instr, $units, $color, $id, $userId);
            mysqli_stmt_execute($stmt);
    ...
    if ($action === 'delete_subject') {
        $id = intval($_POST['subject_id']);
        mysqli_query($conn, "DELETE FROM subjects WHERE id=$id AND user_id=$userId");
...
$subjects = mysqli_query($conn, "
  ...
  FROM subjects s WHERE s.user_id=$userId ORDER BY s.subject_name ASC");

$pageTitle = "Subjects";
$pageSub   = "Manage the courses you're taking this term.";
$activeNav = "subjects";
  ...
  <div class="modal">
    ...
    <form method="POST">
      ...
      <div class="form-row cols-2">
        ...
        <div><label>Color Tag</label><input type="color" name="color" value="#14b8a6" style="height:44px;padding:4px"></div>
      </div>
...
function editSubject(s){
  ...
  document.getElementById('es_color').value = s.color || '#14b8a6';
