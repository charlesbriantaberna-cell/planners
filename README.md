...
> **Updated build:** `backend/` now contains the newer PHP/MySQL
> Student Planner project (adds study groups + group chat, alongside
> tasks, schedule, subjects, notes, calendar, analytics, and 1:1
> chat). The app is back to loading that backend from a hosted URL —
> **you do need to set `app_url`** in
> `app/src/main/res/values/strings.xml` before building (see "Before
> you build" below), and you need to actually host `backend/` on a
> PHP/MySQL server (or run it locally via XAMPP) for the app to work.
> The old fully-offline HTML/JS + localStorage version is still kept
> at `app/src/main/assets/www/` in case that mode is wanted again
> later, but the app no longer loads it by default.
>
...
| Blank white screen when app opens | Check that `app/src/main/assets/www/login.html` exists in the project — that's the file the app loads. |
...
│       ├── assets/www/            ← the bundled static site (what the app loads)
│       └── res/
...
**How the pieces fit together:** `app/src/main/assets/www/` is the
real application now — a static HTML/JS site with its "database" in
the WebView's `localStorage`. The Android app (`app/`) just opens that
bundled site in a full-screen WebView; nothing is hosted anywhere.
`backend/` (the original PHP/MySQL project) is still in this repo
untouched, but the app no longer points at it or needs it.
