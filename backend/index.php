...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'toggle_task') {
    $id = intval($_POST['task_id']);
    $newStatus = $_POST['new_status'] === 'completed' ? 'completed' : 'pending';
    $completedAt = $newStatus === 'completed' ? 'NOW()' : 'NULL';
    mysqli_query($conn, "UPDATE tasks SET status='$newStatus', completed_at=$completedAt WHERE id=$id AND user_id=$userId");
    header("Location: index.php");
    exit;
}

// ── stats ──
$totalTasks     = mysqli_fetch_assoc(mysqli_query($conn, "SELECT COUNT(*) c FROM tasks WHERE user_id=$userId"))['c'];
$completedTasks = mysqli_fetch_assoc(mysqli_query($conn, "SELECT COUNT(*) c FROM tasks WHERE user_id=$userId AND status='completed'"))['c'];
$pendingTasks   = mysqli_fetch_assoc(mysqli_query($conn, "SELECT COUNT(*) c FROM tasks WHERE user_id=$userId AND status!='completed'"))['c'];
$overdueTasks   = mysqli_fetch_assoc(mysqli_query($conn, "SELECT COUNT(*) c FROM tasks WHERE user_id=$userId AND status!='completed' AND due_date IS NOT NULL AND due_date < CURDATE()"))['c'];
$subjectCount   = mysqli_fetch_assoc(mysqli_query($conn, "SELECT COUNT(*) c FROM subjects WHERE user_id=$userId"))['c'];
...
$todayClasses = mysqli_query($conn, "
  ...
  WHERE s.user_id=$userId AND s.day_of_week='$dow' ORDER BY s.start_time ASC");
...
// upcoming events
$events = mysqli_query($conn, "SELECT * FROM events WHERE user_id=$userId AND event_date >= CURDATE() ORDER BY event_date ASC LIMIT 4");

$pageTitle = "Student Planner Dashboard";
$pageSub   = "Welcome back, " . $user['name'] . " To My System.";
$activeNav = "dashboard";
include __DIR__ . '/includes/layout_head.php';
?>

<?php if (isset($_GET['welcome'])): ?><div class="alert alert-ok">🎉 Welcome to Student Planner, <?php echo h($user['name']); ?>! Your account is ready to go.</div><?php endif; ?>
...
<div class="grid-2" style="margin-bottom:22px">
  ...
  <div class="card card-pad">
    ...
    <?php else: while ($t = mysqli_fetch_assoc($upcoming)): ?>
      <div class="flex items-center gap-10" style="padding:11px 0;border-bottom:1px solid rgba(70,150,135,.08)">
        <form method="POST">
          <input type="hidden" name="action" value="toggle_task">
          <input type="hidden" name="task_id" value="<?php echo $t['id']; ?>">
          <input type="hidden" name="new_status" value="completed">
          <button type="submit" class="btn-icon btn-outline" style="width:26px;height:26px;border-radius:50%;border-color:rgba(94,234,212,.45)" title="Mark complete"></button>
        </form>
