...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'delete_event') {
        ...
        mysqli_query($conn, "DELETE FROM events WHERE id=$id AND user_id=$userId");
...
$byDay = [];
$tasksRes = mysqli_query($conn, "SELECT id, title, due_date AS d, 'task' AS kind, priority AS meta FROM tasks WHERE user_id=$userId AND due_date BETWEEN '$monthStart' AND '$monthEnd' AND status != 'completed'");
while ($r = mysqli_fetch_assoc($tasksRes)) { $byDay[$r['d']][] = $r; }
$eventsRes = mysqli_query($conn, "SELECT id, title, event_date AS d, 'event' AS kind, event_type AS meta FROM events WHERE user_id=$userId AND event_date BETWEEN '$monthStart' AND '$monthEnd'");
while ($r = mysqli_fetch_assoc($eventsRes)) { $byDay[$r['d']][] = $r; }

$allEventsList = mysqli_query($conn, "SELECT * FROM events WHERE user_id=$userId ORDER BY event_date DESC LIMIT 30");

$pageTitle = "Calendar";
$pageSub   = "Month view of your tasks and events.";
$activeNav = "calendar";
...
<div class="card card-pad" style="margin-bottom:22px">
  ...
  <div style="display:grid;grid-template-columns:repeat(7,1fr);gap:8px">
    ...
    <?php for ($day=1; $day<=$daysInMonth; $day++):
      ...
      $items = $byDay[$dateStr] ?? [];
    ?>
      <div style="min-height:88px;border-radius:10px;padding:8px;background:<?php echo $isToday?'rgba(20,184,166,.1)':'rgba(255,255,255,.02)'; ?>;border:1px solid <?php echo $isToday?'rgba(94,234,212,.4)':'rgba(70,150,135,.08)'; ?>">
...
<div class="card">
  ...
  <table>
    <thead><tr><th>Event</th><th>Date</th><th>Type</th><th>Description</th><th></th></tr></thead>
    <tbody>
    <?php if (mysqli_num_rows($allEventsList) === 0): ?>
      <tr><td colspan="5"><div class="empty-state"><div class="empty-title">No events yet</div><div class="empty-sub">Add exams, deadlines, or important dates.</div></div></td></tr>
