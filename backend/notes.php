...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'add_note' || $action === 'edit_note') {
        ...
        if ($title === '') {
            ...
            $stmt = mysqli_prepare($conn, "UPDATE notes SET subject_id=?, title=?, content=?, is_pinned=? WHERE id=? AND user_id=?");
            mysqli_stmt_bind_param($stmt, "issiii", $sub2, $title, $content, $pinned, $id, $userId);
            mysqli_stmt_execute($stmt);
    ...
    if ($action === 'delete_note') {
        $id = intval($_POST['note_id']);
        mysqli_query($conn, "DELETE FROM notes WHERE id=$id AND user_id=$userId");
        $message = "Note deleted."; $msgType = "ok";
    }

    if ($action === 'toggle_pin') {
        $id = intval($_POST['note_id']);
        mysqli_query($conn, "UPDATE notes SET is_pinned = 1 - is_pinned WHERE id=$id AND user_id=$userId");
    }
}
...
$notes = mysqli_query($conn, "
  ...
  WHERE n.user_id=$userId ORDER BY n.is_pinned DESC, n.updated_at DESC");

$pageTitle = "Notes";
$pageSub   = "Quick notes and reminders, organized by subject.";
$activeNav = "notes";
...
<div class="stat-grid" style="grid-template-columns:repeat(auto-fill,minmax(260px,1fr))">
  ...
  <div class="card card-pad" style="border-top:3px solid <?php echo h($color); ?>">
    ...
    <div style="font-size:12.5px;color:rgba(210,232,222,.75);line-height:1.6;white-space:pre-wrap;margin:8px 0 14px;max-height:130px;overflow:hidden"><?php echo h($n['content']); ?></div>
