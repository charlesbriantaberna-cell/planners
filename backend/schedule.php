...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'delete_slot') {
        ...
        mysqli_query($conn, "DELETE FROM schedule WHERE id=$id AND user_id=$userId");
...
$slotsRes = mysqli_query($conn, "
  ...
  WHERE sc.user_id=$userId ORDER BY FIELD(sc.day_of_week,'Mon','Tue','Wed','Thu','Fri','Sat','Sun'), sc.start_time");
$slots = [];
...
<?php if (!$hasSubjects): ?>
  <div class="card"><div class="empty-state">
    ...
    <div class="empty-sub">You need at least one subject before you can schedule classes. <a href="subjects.php" style="color:var(--glow2)">Go to Subjects →</a></div>
  </div></div>
<?php else: ?>
